...
## 📍 현재 상태
- **날짜**: 2026-05-03 (v14.8 배치 전송 모드 확정)
- **달성**: 신규 기사의 효율적 색인을 위해 IndexNow 배치 전송(POST) 방식을 최종 채택.
- **수정**: `indexnow_service.py` 배치 전송 로직 재구현 및 "신규 기사 중심 전송" 원칙 수립.
- **상태**: 대량의 신규 URL을 한 번에 전송 가능하도록 시스템 최적화 완료.
- **최근 결정**: 
    - **[v14.8] Batch Mode for New Articles**: 개별 전송의 비효율성을 극복하고 신규 발행된 기사들을 가장 빠르게 색인시키기 위해 POST 방식의 배치 모드를 최종 확정함. 단, 불필요한 중복 전송을 방지하고 신규 데이터에 집중함.
    - **[v14.7] Streaming Mode Restoration (Reverted)**: 한때 스트리밍 모드로 회귀했으나, 효율성 및 실무적 판단에 따라 다시 배치 모드로 전환함.
