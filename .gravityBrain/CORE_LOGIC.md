...
3. **헤더 강화 (User-Agent)**: 
   ...
   - [v1.6] **Throttling 및 안정성 강화**: 개별 URL 통보 사이에 1초의 지연(Sleep)을 두어 검색 엔진의 수신 인지력을 높였으며, `git_sync` 성공 여부와 관계없이 IndexNow 알림을 시도하여 URL 누락을 방지합니다.
4. **연동 검증**: 
...
5. **노후 데이터 클린업 (v3.0)**: [v14.4 추가] NLM studio의 저장 공간 효율성을 위해, 생성된 지 2일이 경과한 노트북을 파이프라인 종료 시점에 자동으로 탐색하여 삭제합니다. `NotebookLMApp.cleanup_old_notebooks`가 이 역할을 담당합니다.
...
5. **알림 설정 일원화 (v14.4)**: [v14.4 추가] `automation/.env`와 루트 `.env`의 충돌을 방지하기 위해 루트의 설정을 유일한 진실의 원천(Source of Truth)으로 삼으며, 모든 알림 모듈이 동일한 봇 토큰과 채널 ID를 사용하도록 일원화했습니다.
6. **정확한 기사 단위 리포팅**: [v14.4 추가] 리포트 내 'Published' 건수를 단순히 Job(노트북) 수가 아닌, 실제로 생성된 개별 기사(Article)의 총합으로 계산하여 운영 가시성을 높였습니다.
